...
if ( ! class_exists( 'x_hub_About' ) ) {
	...
	class x_hub_About {
		 ...
		 *
		 * @var x_hub_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof x_hub_About ) ) {
				self::$instance = new x_hub_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap xhub-about-wrap">';
				...
				echo '<a href="'.esc_url('https://wpthemespace.com/product/xhub-pro/').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
    ...
    return add_query_arg(
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section xhub-section xhub-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .xhub-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section xhub-section xhub-section-support three-col">
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="'.esc_url('https://xhub.wpteamx.com/demos').'" target="_blank" >';
					echo esc_html ('X HUB Pro Live Preview', 'x-hub');
					echo '</a></td></tr>';
        ...
        'activating_string'        => __( 'Activating', 'x-hub' )
        )
    );
